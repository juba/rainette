...
- it is better suited to cluster small "homogeneous" documents : for long texts the
  good practice is first to split them into segments before applying the
  method.
...
Here we will compute 5 clusters with a `min_uc_size` of 15 :
...
computed simple clusterings. Here, we compute them with `min_uc_size` at 10 and 15 :
